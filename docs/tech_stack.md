...
> 아래가 **실제 1차 구현 범위**. §0~§7은 전체 로드맵 참고용.
> 핵심 결정: **API 키 필요한 것·유저별 데이터·백엔드는 전부 제외 → 순수 프론트엔드로 출발.**

### MVP 포함
| 영역 | 범위 | 비고 |
|------|------|------|
| 데이터 | **Binance(암호화폐)만** | 키 불필요 + CORS 허용 → 브라우저 직접 호출 |
| 차트 | 캔들·거래량·MA(5/20/60/120)·기간 선택·줌/스크롤/십자선 | 라이브러리 + MA 자체 계산 |
| 배경 스킨 | **앱에 번들된 정적 이미지 1~2개** + 투명도·fit 모드 | 업로드 없음 |
| 지표 스킨 | 좌표 오버레이·최고/최저 자동감지·캐릭터·말풍선 | 핵심 가치. DOM 오버레이 |
| 스킨 전환 | 박아둔 스킨 토글/적용·해제 | 상태는 Zustand, 필요시 localStorage 메타만 |
| 내보내기 | **PNG** | html-to-image 합성 |

### Backlog (MVP 제외 — 뒤로 미룸)
| 항목 | 미루는 이유 | 부활 시 필요 |
|------|-----------|-------------|
| 주식(미국/국내) | API 키 → 백엔드 강제 | 서버리스 프록시 |
| 마켓플레이스 (공유 스킨) | 공유 데이터 = 서버 | DB + API |
| 이미지 업로드 / 유저별 스킨 저장 | 스토리지·용량 | S3/R2 + DB |
| 인증/로그인 | 유저 식별 불필요 | NextAuth + DB |
| AI 스킨 생성 (Phase 3) | 유료 키·Python | 프록시 + 큐 + Python |
| MP4/비디오 내보내기 | MediaRecorder 한계·무거움 | WebM + ffmpeg.wasm |
