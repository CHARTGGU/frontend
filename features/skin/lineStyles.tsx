...
function renderCatTail(
  ...
  // 3 fluffy tuft circles near the tip, slightly overlapping, radius ~3-4px, centered around point2
  const tufts = [
    { dx: 0, dy: 0, r: 4 },
    { dx: perpX * 3, dy: perpY * 3, r: 3 },
    { dx: -perpX * 3, dy: -perpY * 3, r: 3 },
  ];
  ...
  return (
    <>
      ...
      {stripes.map((s, i) => (
        <line key={i} x1={s.x1} y1={s.y1} x2={s.x2} y2={s.y2} stroke="#A8612A" strokeWidth={1.5} />
      ))}
      {tufts.map((t, i) => (
...
function renderRibbon(
  ...
  return (
    <>
      ...
      <line x1={x1} y1={y1} x2={x2} y2={y2} stroke="white" strokeWidth={1.5} strokeDasharray="6 4" />
      {/* bow knot at point2, rotated to line angle */}
      <g transform={`translate(${x2}, ${y2}) rotate(${angle})`}>
        <polygon points="0,0 14,-8 14,8" fill="#FF6FA0" />
        <polygon points="0,0 -14,-8 -14,8" fill="#FF6FA0" />
...
function renderRainbow(
  ...
  return (
    <>
      ...
      <line x1={x1} y1={y1} x2={x2} y2={y2} stroke={`url(#${gradientId})`} strokeWidth={10} strokeLinecap="round" />
      <HitStroke line={line} onPointerDown={onPointerDown} />
    </>
  );
}
